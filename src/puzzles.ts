// src/puzzles.ts - Puzzle generation and validation logic
...
const ROUNDS = new Map<string, { data: RoundData; createdAt: number }>();
...
function addRound(roundId: string, data: RoundData): void {
  if (ROUNDS.size >= MAX_ROUNDS) {
    const firstKey = ROUNDS.keys().next().value!;
    ROUNDS.delete(firstKey);
  }
  ROUNDS.set(roundId, { data, createdAt: Date.now() });
}
...
export function puzzleNew({
  ...
  if (numOperands < 2) throw { status: 400, detail: 'num_operands must be >= 2' } as ApiError;
  ...
  for (let attempt = 0; attempt < ATTEMPTS; attempt++) {
    ...
    const solution = exprToMinimalValueString(expr);
    const roundId = crypto.randomUUID().replace(/-/g, '');
    ...
    return {
      ...
      target: val,
      numbers: shuffledNumbers,
      solution_expr: showSolution ? solution : null,
      used_numbers: used,
  ...
  throw { status: 500, detail: 'Failed to generate puzzle; try adjusting parameters' } as ApiError;
}
...
export function puzzleRush({ difficulty = 1, decoys = 2 } = {}): PuzzleOut {
  if (difficulty < 1 || difficulty > 12) throw { status: 400, detail: 'difficulty must be 1-12' } as ApiError;
  ...
  throw { status: 500, detail: 'Failed to generate puzzle' } as ApiError;
}
...
export function puzzleCheck({ numbers, expression, target }: { numbers: number[]; expression: string; target?: number }): CheckResult {
  if (!numbers || !expression) throw { status: 400, detail: 'Missing numbers or expression' } as ApiError;
...
export function puzzleReveal(roundId: string): { solution: string } {
  const roundData = getRound(roundId);
  if (!roundData) throw { status: 404, detail: 'Round not found' } as ApiError;
  return { solution: roundData.solution };
}
