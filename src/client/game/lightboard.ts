...
const COLUMN_BOUNDS = [2, 34, 67, 100]
const RIGHT_PADDING = 24
const WIDTH_SAFETY_FACTOR = 0.95
const LINE_HEIGHT_EM = 1.7

function getAvailableWidthPercent(zoneLeftPct: number): number {
  const next = COLUMN_BOUNDS.find(b => b > zoneLeftPct) ?? 92
  return next - zoneLeftPct
}

export function getLightboardFontSize(expr: string, boardWidth: number, zoneLeftPct: number): number {
  const slotW = getAvailableWidthPercent(zoneLeftPct) / 100 * boardWidth
  const targetW = Math.max(1, slotW * WIDTH_SAFETY_FACTOR - RIGHT_PADDING)

  measureContext.font = `600 ${BASE_FONT_SIZE}px Caveat`
  const naturalW = measureContext.measureText(expr).width || 1

  return Math.max(MIN_FONT_SIZE, Math.min(BASE_FONT_SIZE, BASE_FONT_SIZE * targetW / naturalW))
}
...
export function applyLightboardFontSize(el: HTMLElement): void {
  ...
  const offsetY = (BASE_FONT_SIZE - size) * LINE_HEIGHT_EM / 2
...
if (document.fonts && document.fonts.load) {
  document.fonts.load(`600 ${BASE_FONT_SIZE}px Caveat`).then(() => {
