...
interface HomeLbItem {
  text: string;
  color: string;
  glow: string;
}

interface HomeLbFont {
  family: string;
  weight: number;
}
...
const HOME_LB_FONTS: HomeLbFont[] = [
...
function homeLbPickStyle(text: string): { family: string; weight: number; size: number; widthPct: number; heightPct: number } {
...
let homeLbAnimId: number | null = null;
let homeLbMutateTimer: ReturnType<typeof setTimeout> | null = null;
let homeLbStars: any[] = [];
let homeLbStar: any = null;
let homeLbFw: any = null;
let homeLbStartTs: number | null = null;
export let homeLbCfg: HomeLbCfgType | null = null;
...
export function homeLbScaleParams(type: string, params: DoodleParams, wRatio: number, hRatio: number): void {
...
function homeLbBuildParams(type: string, w: number, h: number): DoodleParams {
  switch (type) {
...
function homeLbBuildCfg(w: number, h: number): HomeLbCfgType {
...
function homeLbRandomPositions(
...
function homeLbNewFw(w: number, h: number): any {
...
function homeLbTickFw(ctx: CanvasRenderingContext2D, fw: any, ts: number): boolean {
...
export function showHomeLightboard(): void {
  ...
  const activeItems: { item: HomeLbItem; pos: { x: number; y: number; wPct?: number; hPct?: number }; el: HTMLElement; poolIdx: number }[] = [];
  const usedPool = new Set(selIdx.slice(0, actual));

  function makeTextEl(item: HomeLbItem, pos: { x: number; y: number }, delaySec: number, fontFamily: string, fontWeight: number, fontSize: number): HTMLElement {
